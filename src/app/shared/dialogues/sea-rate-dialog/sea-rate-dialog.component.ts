...
export class SeaRateDialogComponent implements OnInit {
  ...
  cargoTypeChange(type) {
    if (this.transPortMode === "SEA" || this.transPortMode === "GROUND") {
    ...
    try {
      ...
      obj = {
        ...
        carrierID: this.selectedShipping
          ? this.selectedShipping.CarrierID
          : undefined,
        carrierName: this.selectedShipping
          ? this.selectedShipping.CarrierName
          : undefined,
        carrierImage: this.selectedShipping
          ? this.selectedShipping.CarrierImage
          : undefined,
        providerID: this.userProfile.ProviderID,
        containerSpecID:
      ...
      if (
        ...
        this._toast.error("Both Price fields cannot be zero or empty", "Error");
        this.isRateUpdating = false;
        return;
      }
  ...
  getDropdownsList() {
    ...
    if (
      ...
      const groundContainers = this.combinedContainers.filter(
        e =>
          e.ContainerFor === "FTL" && e.ShippingCatID === this.selectedCategory
      );
  ...
  getWarehousePricing() {
    ...
    this._manageRateService.getWarehousePricing("WAREHOUSE").subscribe(
      (res: any) => {
        ...
        console.log(this.fullWarehousePricing);
      },
      err => {
        loading(false);
      }
